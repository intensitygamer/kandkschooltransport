...
class Avada_Breadcrumbs {
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = '<div class="fusion-breadcrumbs" vocab="https://schema.org/" typeof="BreadcrumbList">' . $this->html_markup . '</div>';
	}
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = true, $microdata = true, $is_leaf = false ) {
		...
		if ( $microdata && Avada()->settings->get( 'disable_date_rich_snippet_pages' ) && Avada()->settings->get( 'disable_rich_snippet_title' ) ) {
			$microdata_itemscope = 'property="itemListElement" typeof="ListItem"';
			$microdata_url       = 'property="item" typeof="WebPage"';
			$microdata_title     = 'property="name"';
		}
		...
		if ( $link ) {
			...
			$breadcrumb_content .= '<meta property="position" content="' . $this->item_position . '">';
			$this->item_position++;
		}
